...
class OacRestClient:
    ...
    def create_connection(
        ...
        *,
        ...
        OAC expects a multipart/form-data body with:
          - ``connectionDetails`` field: the 6-key JSON the UI form's "Upload File" button consumes
          - ``privateKey`` file: the ``.pem`` private key matching ``payload.fingerprint``
          - ``name``, ``description``, ``connectionType=Oracle AI Data Platform``

        Args:
            name: Human-readable connection name (e.g. ``aidp_fusion_jdbc``).
            payload: The 6-key payload from
            ...
            private_key_pem_path: Path to the RSA private key whose public half is
                registered on the user OCID (see ``oac_rest_api_setup.md``).
        ...
        with pem_path.open("rb") as pem_fh:
            files = {
                "connectionDetails": ("connection.json", payload.to_json(), "application/json"),
                "privateKey": (pem_path.name, pem_fh.read(), "application/x-pem-file"),
            }
            form_data = {
                "name": name,
                "connectionType": "Oracle AI Data Platform",
            }
            if description:
                form_data["description"] = description
            response = self._request("POST", "/catalog/connections", files=files, data=form_data)
