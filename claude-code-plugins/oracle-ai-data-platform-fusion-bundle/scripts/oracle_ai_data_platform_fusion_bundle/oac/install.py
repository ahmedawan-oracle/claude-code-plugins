...
def _install_via_rest(
    ...
    if not (params.idcs_url and params.client_id and params.client_secret):
        raise ValueError(
            ...
            "(or the equivalent fields in bundle.yaml). Use --print-only to skip "
            "REST and write a JSON file for manual upload."
        )
    ...
    # Connection: create if not present, else skip + report.
    existing = client.find_connection(params.connection_name)
    if existing:
        console.print(
            f"[yellow]Connection '{params.connection_name}' already exists "
            f"(id={existing.get('id') or existing.get('connectionId')}). "
            f"Skipping create. Run [cyan]dashboard uninstall[/cyan] first to recreate.[/yellow]"
        )
        connection_id = str(existing.get("id") or existing.get("connectionId"))
    else:
    ...
    return InstallResult(connection_id=connection_id, imported_workbooks=imported)
