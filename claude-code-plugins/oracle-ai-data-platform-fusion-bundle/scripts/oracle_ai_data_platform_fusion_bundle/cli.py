...
@click.option("--client-id", default=None,
              help="IDCS confidential-app client_id. Required unless --print-only.")
@click.option("--client-secret", default=None,
              help="IDCS confidential-app client_secret, or ${vault:OCID}. "
                   "Required unless --print-only.")
@click.option("--oauth-scope", default="urn:opc:resource:fawcommon:OAC", show_default=True,
              help="OAuth scope published by the IDCS resource server for OAC.")
...
def dashboard_install(
    ...
    Two modes:
      * Default: full REST install (requires --idcs-url, --client-id, --client-secret).
