...
class TestCreateConnection:
    def test_posts_multipart_with_pem(self, tmp_path: Path) -> None:
        ...
        assert "connectionDetails" in files
        assert "privateKey" in files
        # connectionDetails value is the JSON string of the 6-key payload
        assert files["connectionDetails"][0] == "connection.json"
        import json as _json
        loaded = _json.loads(files["connectionDetails"][1])
        assert loaded["username"] == "u"
        assert loaded["idl-ocid"] == "idl"
        # form data carries connectionType so OAC routes it to the AIDP connector
        assert call.kwargs["data"]["connectionType"] == "Oracle AI Data Platform"
        assert call.kwargs["data"]["name"] == "aidp_fusion_jdbc"
